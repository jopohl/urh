...
class ProtocolLabel(object):
    ...
    def __init__(
        ...
        self.display_format_index = (
            0 if field_type is None else field_type.display_format_index
        )
    ...
    def is_preamble(self) -> bool:
        return (
            ...
            and self.field_type.function == FieldType.Function.PREAMBLE
        )
    ...
    def is_sync(self) -> bool:
        return (
            self.field_type is not None
            and self.field_type.function == FieldType.Function.SYNC
        )
    ...
    def display_order_str(self) -> str:
        try:
            ...
            return bit_order + "/{}".format(
                "BE" if self.display_endianness == "big" else "LE"
            )
        except IndexError:
            return ""
    ...
    def __eq__(self, other):
        return (
            ...
            and self.field_type_function == other.field_type_function
        )
    ...
    def overlaps_with(self, other_label):
        return Interval(self.start, self.end).overlaps_with(
            Interval(other_label.start, other_label.end)
        )
    ...
    def to_xml(self) -> ET.Element:
        return ET.Element(
            ...
            attrib={
                ...
                "apply_decoding": str(self.apply_decoding),
                "show": str(self.show),
    ...
    def from_xml(cls, tag: ET.Element, field_types_by_caption=None):
        ...
        result.show = (
            Qt.Checked if Formatter.str2val(tag.get("show", 0), int) else Qt.Unchecked
        )
        result.fuzz_me = (
            Qt.Checked
            if Formatter.str2val(tag.get("fuzz_me", 0), int)
            else Qt.Unchecked
        )
        result.fuzz_values = tag.get("fuzz_values", "").split(",")
        result.auto_created = (
            True if tag.get("auto_created", "False") == "True" else False
        )
