...
class Ui_DialogAdvancedModSettings(object):
    ...
    def retranslateUi(self, DialogAdvancedModSettings):
        ...
        self.label_2.setText(_translate("DialogAdvancedModSettings", "<html><head/><body><p>This setting only affects ASK demodulation.</p><p>With the message divisor length you can configure a <span style=\" font-weight:600;\">minimum message length</span> in a flexible way.</p><p>URH will try to demodulate signals in such a way, that the resulting message has a number of bits that is divisble by the configured divisor.</p></body></html>"))
