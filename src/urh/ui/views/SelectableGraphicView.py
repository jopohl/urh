...
class SelectableGraphicView(QGraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        self.select_all_action.setShortcutContext(Qt.ShortcutContext.WidgetWithChildrenShortcut)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        is_in_shift_mode = (has_shift_modifier and self.hold_shift_to_drag) \
                           or (not has_shift_modifier and not self.hold_shift_to_drag) \
                              and cursor != Qt.CursorShape.SplitHCursor and cursor != Qt.CursorShape.SplitVCursor
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.selection_area.finished and not self.selection_area.resizing:
            ...
            if roi_edge is None:
                if (cursor == Qt.CursorShape.SplitHCursor and self.has_horizontal_selection) \
                        or (cursor == Qt.CursorShape.SplitVCursor and not self.has_horizontal_selection):
                    self.unsetCursor()
                    return
        ...
        if event.buttons() == Qt.MouseButton.LeftButton and self.selection_area.resizing:
        ...
        self.mouse_pos = event.pos()
        if event.buttons() == Qt.MouseButton.LeftButton and not self.selection_area.finished:
