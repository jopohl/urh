...
class TestSimulatorTabGUI(QtTestCase):
    ...
    def test_edit_simulator_label_table(self):
        ...
        self.assertNotEqual(
            model.data(model.index(0, 3), role=Qt.ItemDataRole.BackgroundRole),
            settings.ERROR_BG_COLOR,
        )
        model.setData(
            model.index(0, 3), "item1.preamble + 42", role=Qt.ItemDataRole.EditRole
        )
        self.assertNotEqual(
            model.data(model.index(0, 3), role=Qt.ItemDataRole.BackgroundRole),
            settings.ERROR_BG_COLOR,
    ...
    def test_simulator_graphics_view(self):
        ...
        stc.ui.gvSimulator.contextMenuEvent(
            QContextMenuEvent(QContextMenuEvent.Reason.Mouse, pos)
        )
