...
class Ui_DialogAdvancedModSettings(object):
    ...
    def retranslateUi(self, DialogAdvancedModSettings):
        ...
        DialogAdvancedModSettings.setWindowTitle(
            _translate("DialogAdvancedModSettings", "Advanced Modulation Settings")
        )
        self.groupBox.setTitle(
            _translate("DialogAdvancedModSettings", "Pause Threshold")
        )
        self.label.setText(
            _translate(
                "DialogAdvancedModSettings",
                '<html><head/><body><p>The pause threshold gives you control <span style=" font-weight:600;">when to insert a message break.</span></p><p>The pause threshold is the maximum length of consecutive zero bits represented by a pause before a new message begins.</p><p>Special value is 0 to disable message breaking completely.</p></body></html>',
            )
        )
        self.spinBoxPauseThreshold.setSpecialValueText(
            _translate("DialogAdvancedModSettings", "Disable")
        )
        self.groupBox_2.setTitle(
            _translate("DialogAdvancedModSettings", "Message Length Divisor")
        )
