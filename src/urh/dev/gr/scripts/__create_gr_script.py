    ...
    with open("airspy_recv.py", "w") as f:
        for line in r:
            ...
            if line.strip().startswith("def __init__"):
                f.write(line.replace("self", "self, " + ", ".join(list(filter(None, used_variables)))))
                continue
            ...
            if line.strip().startswith("def main("):
                ...
                f.write("parser = OptionParser(option_class=eng_option, usage='%prog: [options]')\n")
                f.write("parser.add_option('-s', '--samplerate', dest='samplerate', default=100000)\n")
                f.write("parser.add_option('-f', '--frequency', dest='frequency', default=433000)\n")
                f.write("parser.add_option('-g', '--gain', dest='gain', default=30)\n")
